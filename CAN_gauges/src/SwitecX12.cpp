...
SwitecX12::SwitecX12(unsigned int steps, unsigned char pinStep, unsigned char pinDir, bool invert)
{
  this->steps = steps;
...
{
  if (invert) { digitalWrite(pinDir, dir > 0 ? HIGH : LOW); }
  else { digitalWrite(pinDir, dir > 0 ? LOW : HIGH); }
